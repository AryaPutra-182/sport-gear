  ...
  const handleDelete = async (id: number) => {
    setModal({
      ...
      action: async () => {
        ...
        if (res.error) {
          setModal({ open: true, msg: "Gagal hapus: " + res.error });
          return;
        }
        setProducts((prev) => prev.filter((p) => p.id !== id));
        router.refresh();
        setModal({ open: true, msg: "Produk berhasil dihapus!" });
      },
    });
  };
    ...
    try {
      ...
      if (res.error) {
        ...
        if (editingId) {
          ...
          setModal({open:true, msg:"Produk diperbarui!"});
        } else {
          // Logic Tambah: Masukkan item baru ke paling atas
          setProducts((prev) => [newData, ...prev]);
          setModal({open:true, msg:"Produk ditambahkan!"});
        }
      ...
      console.error(error);
      setModal({open:true, msg:"Terjadi kesalahan sistem"});
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="space-y-6">
        ...
        <table className="w-full text-left text-gray-600">
          ...
          <tbody className="divide-y divide-gray-100">
            {filteredProducts.map((product) => (
              <tr
                key={product.id}
                className="hover:bg-[#F7F5E9] transition-colors"
              >
                <td className="p-4">
                ...
                <td className="p-4">
                  <div className="flex justify-center gap-3">
                    <button
                      ...
                      className="text-gray-400 hover:text-[#F4B400] transition-colors p-1"
                      title="Edit"
                    >
          ...
          <div className="bg-white rounded-xl w-full max-w-lg border border-gray-200 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            >
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-[#122D4F] hover:bg-[#0C2E4E] text-white font-bold py-3.5 rounded-lg transition-all shadow-md mt-2 disabled:opacity-50"
              >
      ...
      `}</style>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
              <div className="bg-white w-full max-w-sm rounded-2xl p-8 shadow-xl border border-gray-200 animate-fadeIn">
                ...
                <button
                  onClick={() => {
                    ...
                    setModal({ open: false, msg: "", action: undefined });
                  }}
